...
class DataAccess:
    ...
    def match_scenario_files(self, scenario_id, kind):
        ...
        :return: (*str*) -- the specified path
        """
        base_dir = self.get_base_dir(kind)
        if kind == "tmp":
    ...
    def copy(self, src, dest, recursive=False):
        ...
        """
        if recursive:
            self.fs.copydir(src, dest)
        else:
            # TODO: check if dest needs to be a directory
